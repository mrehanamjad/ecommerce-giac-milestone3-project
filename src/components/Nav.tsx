      ...
      <Container>
        <div className="w-full h-full flex justify-between items-center">
          <div className="h-6 text-text2  flex items-center justify-between gap-48">
            <Logo />
            <div className="flex items-center justify-center gap-4 font-">
              {navItems.map((item) => (
                <Link
                  key={item.slug}
                  href={item.slug}
                  className={`ml-4  font-sans text-Title-16PX-Regular text-text2 text-center align-top  hover:text-gray-900 `}
                >
                  {item.name}
                </Link>
              ))}
          ...
          <div className="w-[21.6875rem] h-[2.375rem] flex  items-center space-x-4">
            <div className="py-[0.4375rem] pl-5 pr-3 h-full bg-secondary flex justify-center items-center gap-1 w-[15.2rem] ">
              <input
